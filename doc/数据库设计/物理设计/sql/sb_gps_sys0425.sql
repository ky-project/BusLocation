...
(
  `ID` int(11) NOT NULL AUTO_INCREMENT COMMENT '设定自增_',
  `GPS_ID` varchar(100) NOT NULL comment 'GPS信号中自带的识别码作为主键',
...
alter table P_NOTICE add constraint FK_Relationship_12 foreign key (USER_ID)
      references SYS_USER (ID) on delete restrict on update restrict;

alter table P_NOTICE_ATTACH add constraint FK_Relationship_13 foreign key (NOTICE_ID)
      references P_NOTICE (ID) on delete restrict on update restrict;

alter table P_NOTICE_ATTACH add constraint FK_Relationship_14 foreign key (FILE_ID)
      references FILE_ATTACH (ID) on delete restrict on update restrict;

alter table SB_BUSPOSITION add constraint FK_Relationship_24 foreign key (GPS_ID)
      references SB_GPS (ID) on delete restrict on update restrict;

alter table SB_BUSPOSITION_HIS add constraint FK_Relationship_25 foreign key (GPS_ID)
      references SB_GPS (ID) on delete restrict on update restrict;

alter table SB_BUS_ROUTE add constraint FK_Relationship_6 foreign key (BUS_ID)
      references SB_BUS (ID) on delete restrict on update restrict;

alter table SB_BUS_ROUTE add constraint FK_Relationship_7 foreign key (ROUTE_ID)
      references SB_ROUTE (ID) on delete restrict on update restrict;

alter table SB_GPS add constraint FK_Relationship_20 foreign key (BUS_ID)
      references SB_BUS (ID) on delete restrict on update restrict;

alter table SB_ROLE_AUTHORITY add constraint FK_Relationship_21 foreign key (ROLE_ID)
      references SYS_ROLE (ID) on delete restrict on update restrict;

alter table SB_ROLE_AUTHORITY add constraint FK_Relationship_22 foreign key (AUTHORITY_ID)
      references SYS_AUTHORITY (ID) on delete restrict on update restrict;

alter table SB_ROUTE_STATION add constraint FK_Relationship_8 foreign key (ROUTE_ID)
      references SB_ROUTE (ID) on delete restrict on update restrict;

alter table SB_ROUTE_STATION add constraint FK_Relationship_9 foreign key (STATION_ID)
      references SB_STATION (ID) on delete restrict on update restrict;

alter table SB_USER_BUS add constraint FK_Relationship_16 foreign key (USER_ID)
      references SYS_USER (ID) on delete restrict on update restrict;

alter table SB_USER_BUS add constraint FK_Relationship_17 foreign key (ROUTE_ID)
      references SB_ROUTE (ID) on delete restrict on update restrict;

alter table SB_USER_ROLE add constraint FK_Relationship_15 foreign key (USER_ID)
      references SYS_USER (ID) on delete restrict on update restrict;

alter table SB_USER_ROLE add constraint FK_Relationship_18 foreign key (ROLE_ID)
      references SYS_ROLE (ID) on delete restrict on update restrict;

alter table SYS_USER add constraint FK_Relationship_19 foreign key (DEPARTMENT_ID)
